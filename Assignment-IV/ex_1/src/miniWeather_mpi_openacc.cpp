...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  // Apply the tendencies to the fluid state
    // #pragma acc parallel loop collapse(3) copyin(state_init[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])  \
    //                                       copyin(tend[0:nx*nz*NUM_VARS]) \
    //                                       copyout(state_out[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) private(inds, indt) \
    //                                       private(inds, indt)
    #pragma acc parallel loop collapse(3) copy(state_init[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])  \
                                        copy(tend[0:nx*nz*NUM_VARS]) \
                                        copy(state_out[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) private(inds, indt) \
                                        private(inds, indt)
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  #pragma acc data copyin(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) \
                   copyout(flux[0:(nx+1)*(nz+1)*NUM_VARS]) \
                   copyout(tend[0:nx*nz*NUM_VARS])
  {
...
void set_halo_values_x( double *state ) {
  ...
  #pragma acc data copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS], sendbuf_l[0:hs*nz*NUM_VARS], sendbuf_r[0:hs*nz*NUM_VARS])
  {
...
void set_halo_values_z( double *state ) {
  ...
  #pragma acc parallel loop collapse(2) private(x, xloc, mnt_deriv) \
                   copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
