                                 ...
                                 struct grid grd)
{
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&Jy_gpu[get_idx(ix-ii, iy-jj, iz-kk, grd.nyn, grd.nzn)], temp[ii][jj][kk] * grd.invVOL);
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&Jz_gpu[get_idx(ix-ii, iy-jj, iz-kk, grd.nyn, grd.nzn)], temp[ii][jj][kk] * grd.invVOL);
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&pxx_gpu[get_idx(ix-ii, iy-jj, iz-kk, grd.nyn, grd.nzn)], temp[ii][jj][kk] * grd.invVOL);
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&pxy_gpu[get_idx(ix-ii, iy-jj, iz-kk, grd.nyn, grd.nzn)], temp[ii][jj][kk] * grd.invVOL);
    ...
    for (int ii = 0; ii < 2; ii++)
        for (int jj = 0; jj < 2; jj++)
            for (int kk = 0; kk < 2; kk++)
                atomicAdd(&pyy_gpu[get_idx(ix-ii, iy-jj, iz-kk, grd.nyn, grd.nzn)], temp[ii][jj][kk] * grd.invVOL);
...
void interpP2G_gpu(struct particles* part , struct interpDensSpecies* ids, struct grid* grd, FPpart* part_x_gpu  , 
                    ...
                    FPfield* XN_flat_gpu   , FPfield* YN_flat_gpu  , FPfield* ZN_flat_gpu  , long grd_size){
    ...
    interpP2G_kernel<<<(n_particles+TPB-1)/TPB, TPB>>>(part_x_gpu  , part_y_gpu  , part_z_gpu  ,
                                                       part_u_gpu  , part_v_gpu  , part_w_gpu  , 
                                                       part_q_gpu  , XN_flat_gpu , YN_flat_gpu , 
                                                       ZN_flat_gpu , rhon_gpu    , Jx_gpu      , 
                                                       Jy_gpu      , Jz_gpu      , pxx_gpu     , 
